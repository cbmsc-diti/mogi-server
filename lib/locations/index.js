...
app.get('/users/:id/dates/enabled', auth.ensureAdmin, function(req,res) {
    var where = ['"userId"= ?'],
        params = [],
        select = ['SELECT CAST(date AS DATE) ENABLED_DATES FROM locations'];
    params.push(req.params.id);
    var query = select.join(' ') + ((where.length > 0) ? ' WHERE ' + where.join(' AND ') : '') + ' GROUP BY CAST(date AS DATE) ORDER BY CAST(date AS DATE) ASC';
